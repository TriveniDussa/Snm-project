...
def register():
    if request.method=='POST':
        ...
        try:
            ...
            cursor.execute('select count(*) from users where usermail=%s',[usermail])
            ...
            if count_email[0]==0:
                ...
                # return server_otp
                userdata={'username':username,'usermail':usermail,'userpassword':userpassword,'server_otp':server_otp}
    ...
    if request.method=='POST':
        ...
        try:
            ...
            if user_data['server_otp']==user_otp:
                #database
                cursor=mydb.cursor()
                cursor.execute('insert into users(username,usermail,userpassword) values(%s,%s,%s)',[user_data['username'],user_data['usermail'],user_data['userpassword']])
                mydb.commit()
...
def login():
    if request.method=='POST':
        ...
        try:
            cursor=mydb.cursor(buffered=True)
            cursor.execute('select count(*) from users where usermail=%s',[login_useremail])
            count_email=cursor.fetchone()   #(1,) or (0,)
            # cursor.close()
        except Exception as e:
            print(e)
            ...
            if count_email[0]==1:
                cursor.execute('select userpassword from users where usermail=%s',[login_useremail])
...
def search():
    ...
    try:
        ...
        cursor.execute(
            'SELECT userid FROM users WHERE usermail=%s',
            (session.get('user'),)
        )
        user_id = cursor.fetchone()
